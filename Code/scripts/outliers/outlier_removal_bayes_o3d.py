...
class TemplateClassifier(BaseEstimator, ClassifierMixin):
    ...
    def score(self, _, __):
        ...
        for i, tmp_las in enumerate(self.loaded_files):
            ...
            file = open('results_outliers.txt', 'a')
            items = [i, self.all_paths[i], pct_data_kept, pct_wires_removed, self.get_params()]
        ...
        alpha = 0.8
        denom =1-(np.sum(np.array(all_pct_wires_removed) == 0)/len(all_pct_wires_removed))
        score = max(0, 1-((1-alpha)*np.mean(all_pct_data_kept))-(alpha*denom))
